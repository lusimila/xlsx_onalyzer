...
def filter_excel(frames: tuple[pd.DataFrame]) -> pd.DataFrame:
    """Фильтрация записей в таблицах согласно условиям:
        1. Проверка наличия даты в колонке 'ГУ "НА"'

        2 Проверка наличия года в колонке 'ИМНС'

        2.1 При наличии в колонке 'ИМНС' года сравнивается с текущим на разницу в 4 года

        2.2 При отсутствии в колонке 'ИМНС' года, сравнивает год из колонки
            'Дата прекращения' с текущим на разницу в 11 лет

        3 Формируется таблица DataFrame с результатом

    :param frames: Таблицы для фильтрации
    :return: Таблица результата в DataFrame
    """
...
def filter_year(year: int, frames: tuple[pd.DataFrame]) -> pd.DataFrame:
    """Фильтрация записей в таблицах по году
