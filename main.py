...
from os import listdir    # для того, чтобы рассмотреть, какие эксель файлы есть в папке
...
# импортируем CONFIG из файла setting. CONFIG является у нас словарем,

from settings import CONFIG

# объявляем функцию find_excel_files, она будет находить все excel файлы в нашей
...
# функция  read_excel считывает все найденные excel файлы и превратила их в таблицы (dataframe)
...
def read_excel(excel_files: tuple[str]) -> tuple[pd.DataFrame]:
    ...
    for excel in excel_files:
        ...
        # если листа не окажется произойдет ошибка, except уловит это ошибку для предотвращения падения программы
        except:
            pass
    # возвращаем кортеж таблиц (dataframe)
    return tuple(frames) # превращаем список таблиц (dataframe) в кортеж таблиц (dataframe) и возвращаем его
...
# Далее  проводятся:  фильтрация, перебор записей на наличие дат в колонке "ИМНС", сверке дат,
