...
def find_excel_files() -> tuple[str]:  # функция будет возвращать кортеж строк
    ...
    :return: Список имен файлов XLSX
    """
...
def read_excel(excel_files: tuple[str]) -> tuple[pd.DataFrame]:
    """Считывание интересующих книг из файлов XLSX

    :param excel_files: Имена файлов XLSX
    :return: Интересующие книги в виду таблиц DataFrame
    """
...
def filter_excel(frames: tuple[pd.DataFrame]) -> pd.DataFrame:
    """Фильтруем записи в таблицах согласно условиям:
        1. Проверяет наличие даты в колонке 'ГУ "НА"'
        2 Проверяет наличие года в колонке 'ИМНС'
        2.1 Если в колонке 'ИМНС' присутствует год, то сравнивает его с текущим на разницу в 4 года
        2.2 Если в колонке 'ИМНС' не присутствует год, то сравнивает год из колонки
            'Дата прекращения' с текущим на разницу в 11 лет
        3 Формирует таблицу DataFrame с результатом

    :param frames: Таблицы для фильтрации
...
def filter_year(year: int, frames: tuple[pd.DataFrame]) -> pd.DataFrame:
    """Фильтруем записи в таблицах по году

    :param year: Год для фильтрации
    :param frames: Таблицы для фильтрации
    :return: Таблицу результата
    """
    result = {
        CONFIG['registration_number']: [],
        CONFIG['documents_count']: [],
...
def write_result(frame: pd.DataFrame, file_name: str) -> None:
    """Запись таблицы DataFrame в файл XLSX

    :param frame: Таблица для записи
    :param file_name: Имя файла дл записи
    """
    frame.to_excel(f'{file_name}.xlsx', sheet_name=file_name) # 1 arg - имя файла для записи, 2 arg - имя книги для записи
